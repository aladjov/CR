...
generator = PipelineGenerator(
    ...
    experiments_dir="./experiments",            # Delta tables from exploration
    production_dir="./production",             # Production output (defaults to experiments_dir)
)
...
├── gold/
│   └── gold_features.py         # Feature engineering + transforms
├── training/
│   └── ml_experiment.py         # Model training with MLFlow
├── scoring/
│   └── run_scoring.py           # Scoring pipeline (fit_mode=False)
├── validation/
│   └── validate_pipeline.py     # Scoring validation gates
├── feast_repo/                  # Feature store definitions
├── exploration_report.md        # Generated documentation
...
The `training/ml_experiment.py` generated by the pipeline includes MLFlow integration and provenance comments linking back to exploration notebooks:

```python
# Generated code in training/ml_experiment.py
# ─────────────────────────────────────────────────────────
# Provenance: Generated from exploration findings
#   Source notebooks: 01-10 (exploration_notebooks/)
#   Findings: experiments/findings/customers_findings.yaml
#   Pipeline: churn_prediction (run_id: a1b2c3d4)
# ─────────────────────────────────────────────────────────
import pandas as pd
...
def run_experiment():
    ...
    # Metrics logged automatically when run through run_all.py
...
Generated pipelines use `TransformExecutor` with `ArtifactStore` to ensure training and scoring apply identical transformations:

```python
# In gold/gold_features.py (generated)
...
from config import ARTIFACTS_DIR, FIT_MODE

executor = TransformExecutor()

if FIT_MODE:
    # Training: fit transformers and save artifacts
    store = ArtifactStore(ARTIFACTS_DIR)
    df = executor.apply_all(df, transform_steps, fit_mode=True, artifact_store=store)
    store.save_manifest()
else:
    # Scoring: load saved transformers and replay
    store = ArtifactStore.from_manifest(ARTIFACTS_DIR / "manifest.yaml")
    df = executor.apply_all(df, transform_steps, fit_mode=False, artifact_store=store)
```

The `manifest.yaml` records every fitted artifact:
...
contract_type_encode:
  ...
  path: ./artifacts/contract_type_encode.joblib
```
...
# ============================================
# STEP 7: Validate scoring pipeline
# ============================================
...
report = compare_pipeline_outputs(
    training_output_path="./orchestration/churn_prediction/data/gold",
    scoring_output_path="./orchestration/churn_prediction/data/scoring",
    entity_column="customer_id",
    target_column="churn",
    output_report_path="./orchestration/churn_prediction/validation_report.yaml",
)
print(f"Validation: {'PASSED' if report.passed else 'FAILED'}")
print(f"Feature mismatches: {len(report.feature_mismatches)}")
```
...
| **Test locally first** | Run `run_all.py` locally before deployment |
...
| **Write to Delta** | Use Delta Lake for all layer outputs — get ACID transactions and time travel for free |
| **Validate before deploy** | Run `compare_pipeline_outputs()` to catch train/serve skew before production |
| **Check provenance** | Generated code includes comments linking back to exploration notebooks and findings |
