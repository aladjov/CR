...
class TestMLflowAliasInterface:
    def test_set_alias_is_abstract(self):
        from customer_retention.integrations.adapters.mlflow import MLflowAdapter

        assert "set_alias" in MLflowAdapter.__abstractmethods__

    def test_get_model_by_alias_is_abstract(self):
        from customer_retention.integrations.adapters.mlflow import MLflowAdapter

        assert "get_model_by_alias" in MLflowAdapter.__abstractmethods__


class TestDatabricksMLflowAlias:
    def test_set_alias_calls_client(self):
        from unittest.mock import MagicMock, patch

        from customer_retention.integrations.adapters.mlflow import DatabricksMLflow

        with patch("customer_retention.integrations.adapters.mlflow.databricks.is_spark_available", return_value=True):
            adapter = DatabricksMLflow()
        adapter._client = MagicMock()
        adapter.set_alias("my_model", "champion", "1")
        adapter._client.set_registered_model_alias.assert_called_once_with(
            name="my_model", alias="champion", version="1"
        )

    def test_get_model_by_alias_returns_version(self):
        from unittest.mock import MagicMock, patch
        ...
        adapter._client = MagicMock()
        adapter._client.get_model_version_by_alias.return_value = mock_version
        result = adapter.get_model_by_alias("my_model", "champion")
        assert result == mock_version


@requires_mlflow
class TestLocalMLflowAlias:
    def test_set_alias_calls_client(self, tmp_path):
    ...
    def test_get_model_by_alias_returns_version(self, tmp_path):
        from unittest.mock import MagicMock
...
class TestExperimentTrackerSearch:
    def test_list_exploration_runs(self, tmp_path):
        ...
        tracker = ExperimentTracker(tracking_uri=str(tmp_path / "mlruns"))
        findings = ExplorationFindings(source_path="test.csv", source_format="csv", row_count=100, column_count=3)
