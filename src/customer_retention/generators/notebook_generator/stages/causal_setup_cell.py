...
- **`SHAP_METHOD`** — `"linear"` (default, fast) emits distributed linear attribution `importance × (x − mean)` using the importance vector and background means **persisted at model-training time** as an MLflow run artifact (`shap_attribution.json`). No scoring is re-run here: the attribution is a pure function of (frozen model, training cohort) and is loaded once per derivation. Exact for LogisticRegression, first-order approximation for tree ensembles. `"sampling"` computes true Shapley values via Štrumbelj–Kononenko Monte Carlo sampling on top of `mlflow.pyfunc.spark_udf` — model-agnostic, higher cost (O(N × K × M) predictions).
